    ...
    >
      ...
      >
        <motion.h2 
          ...
          variants={fadeInUp}
        >
          <span className="bg-gradient-to-r from-cyan-400 to-blue-600 bg-clip-text text-transparent">{ABOUT_CONTENT.title}</span>
        </motion.h2>
            ...
            {ABOUT_CONTENT.cards.map((card, index) => (
              ...
              >
                ...
                {card.items.map((item, itemIndex) => (
                  <p key={itemIndex} className={`text-sm ${itemIndex === 0 ? 'text-gray-300' : 'text-gray-400'}`}>
                    {item}
                  </p>
                ))}
              </motion.div>
